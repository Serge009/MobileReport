        ...
        fileSystem.root.getFile(fileSource, {create: false},
            function (file_entry) {
                if (element.deleted != 1) {
                    ...
                    list += '<div id=' + element.id + ' class="elem-list ' + class_name + '">' +
                    '<img src="' + $PATH + element.logo_path + '" />' +
                    '<h2>' + element.group_name.slice(0, 13) + '</h2>' +
                    '</div>';
        ...
        fileSystem.root.getFile(fileSource, {create: false},
            function (file_entry) {
                if (element.graph_group_id == category && element.deleted != 1) {
                    list += '<div class="category" data-id="' + element.id + '" data-type="' + element.type + '">' +
                    '<img src="' + $PATH + element.logo_path + '" />' +
                    '<h4 style="color: white;">' + element.graph_name.slice(0, 23) + '</h4>' +
                    '</div>';
                }
                setTypesList(index);
